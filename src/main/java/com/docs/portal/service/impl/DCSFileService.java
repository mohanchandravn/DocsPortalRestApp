...
public class DCSFileService extends DocumentService {
	...
	public FileUploadReponse uploadFile(MultipartFile multipartFile, String companyName, String invoiceNumber) {
		...
		if (null != multipartFile) {
			...
			// Metadata values
			String metadataValues = "{ \"collection\" : " + collectionName + "," + METADATA_FIELD_CUSTOMER_NAME + " : "
					+ companyName + "," + METADATA_FIELD_INVOICE_NUMBER + " : " + invoiceNumber + "}";
			...
			fileDataBodyPart.setContentDisposition(
					FormDataContentDisposition.name("primaryFile").fileName(fileToUpload.getName()).build());
			formData.bodyPart(fileDataBodyPart);

			formData.setMediaType(MediaType.MULTIPART_FORM_DATA_TYPE);

			String responseString = oServicesHelper.executePost(docsURL, headers, MediaType.MULTIPART_FORM_DATA_TYPE,
					formData);
	...
	public Map<String, Object> downLoadFile(String fileId, String version) {
		...
		Map<String, Object> responseMap = oServicesHelper.executeGetForDownload(docsURL, queryParams, headers,
